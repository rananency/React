...
function Add_Task() {
    ...
    const vali_title = (fname) => {
        var today = new Date()
        if (fname === '') {
            setTaskData({ ...Task_data, errmsg: 'Title is required!!' })
        }
        else if (moment(Task_data.dueDate).format('DD-MMM-YYYY') < moment(today).format('DD-MMM-YYYY')) {
            setTaskData({ ...Task_data, errmsg: 'Due Date must be Greater or Equal than today\'s Date' })
        }
        else {
    ...
    const vali_date = () => {
        ...
        alert(moment(today).format('DD-MMM-YYYY'))
        if (moment(Task_data.dueDate).format('DD-MMM-YYYY') < moment(today).format('DD-MMM-YYYY')) {
            setTaskData({ ...Task_data, errmsg: 'Due Date must be Greater or Equal than today\'s Date' })
        }
        else {
            setTaskData({ ...Task_data, errmsg: '' })
                    ...
                    <div className="add-todo-input">
                        ...
                        <div >
                            <input
                                ...
                                onBlur={(e) => vali_title(e.target.value)}
